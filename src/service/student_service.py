...
from src.schemas.student_schemas import SStudentCreate
from src.models.student_models import Student, Course, student_course
from src.service.courses_service import (
    find_existing_courses,
    create_new_courses,
    find_or_create_courses,
)
...
async def add_student(session: AsyncSession, student_data: SStudentCreate) -> Student:
    try:
        ...
        session.add(student)
        logger.info(f"Запись {student.name} успешно добавлена в базу данных")
        await session.flush()
...
async def find_one_with_id(session: AsyncSession, id: uuid.UUID) -> Student:
    try:
        ...
        if not record:
            raise NotFoundError(f"Студент с id {id} не найден")
...
async def update_student_with_course(
    ...
    logger.info(f"Запись {student.name} успешно найдена в базе данных")
    try:
        student.name = student_data.name
        student.courses = await find_or_create_courses(session, student_data.courses)
        logger.info(f"Запись {student.name} успешно обновлена в базе данных")
...
async def delete_student(session: AsyncSession, student_id: uuid.UUID):
    ...
    try:
        ...
        logger.info(f"Запись {student.name} успешно удалена из базы данных")
        return student
    except SQLAlchemyError as e:
