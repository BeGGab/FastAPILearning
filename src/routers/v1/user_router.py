...
async def find_user_is_id(
    id: uuid.UUID, session: AsyncSession = Depends(get_async_session)) -> SUserRead:
...
async def create_user_with_profile(payload: SUserCreate, session: AsyncSession = Depends(get_async_session)) -> SUserRead:
    user = await UserDAO.create_user_with_profile(
        session=session, user_data=payload.model_dump())
    return user
...
async def update_user_profile(
    id: uuid.UUID,
    userupdate: SUserCreate,
    session: AsyncSession = Depends(get_async_session),
) -> SUserRead:
    user = await UserDAO.update_user(
        session=session, user_id=id, **userupdate.model_dump()
    )
    return user
